...
async function getChainIdFromAccessNode() {
  const response = await sdk__namespace.send([sdk__namespace.getNetworkParameters()]).then(sdk__namespace.decode);
...
async function getChainId() {
  let network = await config.config.get("flow.network.default");
  if (!network) {
    network = await setChainIdDefault();
    if (!network) {
      network = await config.config.get("flow.network");
      if (network) {
        ...
        network = await config.config.get("env");
...
function watchForChainIdChanges() {
  return config.config.subscribe(function configSubscriber(config) {
...
async function pre(type, opts) {
  ...
  // prettier-ignore
  utilInvariant.invariant(opts.cadence || (await sdk__namespace.config().get("flow.network")), `${type}(opts) -- Required value for "flow.network" not defined in config. See: ${"https://github.com/onflow/flow-js-sdk/blob/master/packages/fcl/src/exec/query.md#configuration"}`);
  // prettier-ignore
...
const spawnProviders = () => utilActor.spawn(HANDLERS$3, SERVICE_ACTOR_KEYS.AUTHN);
...
const spawnCurrentUser = () => utilActor.spawn(HANDLERS$2, NAME);
...
const spawnEvents = key => utilActor.spawn(HANDLERS, key);
